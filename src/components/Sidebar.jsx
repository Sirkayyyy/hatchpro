...
import { MdDashboard, MdAdd, MdList, MdEgg, MdFlashOn, MdPets, MdCheckCircle, MdBarChart, MdViewKanban, MdInsights, MdSettings } from 'react-icons/md'
  ...
  return (
    <div className={`sidebar ${open ? '' : 'sidebar-hidden'}`}>
      <div className="sidebar-logo">
        <h1 style={{ fontSize: 18, fontWeight: 700, color: '#fff', letterSpacing: '-0.3px' }}>🐣 HatchPro</h1>
        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', letterSpacing: '0.5px', textTransform: 'uppercase' }}>Hatchery Management</span>
      </div>
      <nav style={{ flex: 1, padding: '12px 0' }}>
        <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', padding: '12px 20px 4px' }}>Menu</div>
        {NAV.map(({ path, label, Icon }) => (
          <div key={path} className={`nav-item ${pathname === path ? 'active' : ''}`} onClick={() => navigate(path)}>
