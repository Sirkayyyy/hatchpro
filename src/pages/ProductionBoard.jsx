...
export default function ProductionBoard() {
  ...
  const moveToStage = (id, toStage) => {
    ...
    if (toIdx !== fromIdx + 1) { toast.error('Batches can only advance to the next stage'); return }
    setBatches((b) =>
      b.map((x) =>
        x.id === id
          ? {
  ...
  return (
    <div>
      <PageTitle
        ...
        subtitle="Kanban view — drag cards or use arrows to advance batches"
      />

      <div style={{ background: '#EDE7F6', border: '1px solid #CE93D8', borderRadius: 8, padding: '10px 14px', marginBottom: 20, fontSize: 12, color: '#6A1B9A', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 16 }}>💡</span>
        Drag a card to the next column to advance it, or use the arrow button. Batches move forward only: Incubation → Candling → Hatching → Completed.
      </div>
        ...
        {STAGES.map((stage) => {
          ...
          return (
            <div
              ...
              style={{ outline: isOver ? `2px dashed ${stage.color}` : 'none', outlineOffset: 2, background: isOver ? stage.bg : '#F9FAFB', transition: 'all 0.15s' }}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <span style={{ width: 22, height: 22, borderRadius: '50%', background: stage.countBg, color: stage.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700 }}>
                  {items.length}
                </span>
              </div>

              {items.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 12px', color: 'var(--text3)', fontSize: 12, border: `2px dashed ${stage.countBg}`, borderRadius: 8 }}>
                  {isOver ? `Drop here → ${stage.label}` : 'No batches'}
                </div>
              ) : (
                items.map((b) => {
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
                        ...
                        {nextStage && (
                          <button
                            ...
                            style={{ background: stage.bg, border: `1px solid ${stage.countBg}`, borderRadius: 6, padding: '3px 7px', cursor: 'pointer', fontSize: 11, color: stage.color, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 3 }}
                          >
                            → {nextLabel}
                          </button>
                        )}
                      ...
                      {b.stage === 'incubation' && (
                        <div style={{ fontSize: 10, color: 'var(--text3)' }}>
                          📅 Loaded: {b.dateLoaded || '—'}
                          {b.chicksExpected > 0 && ` · 🐣 ${b.chicksExpected.toLocaleString()} exp.`}
                        </div>
                      )}
                      {b.stage === 'candling' && (
                        <div style={{ fontSize: 10, color: 'var(--text3)' }}>
                      ...
                      {b.stage === 'completed' && (
                        <div style={{ fontSize: 10, color: 'var(--text3)' }}>
                          🐣 Hatched: {(b.chicksHatched || 0).toLocaleString()} · 📊 {b.hatchRate ?? 0}%
                        </div>
                      )}
                      {b.expectedHatch && b.stage !== 'completed' && (
                        <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 3 }}>
                          🎯 Hatch: {b.expectedHatch}
                        </div>
                      )}
                    </div>
                  )
