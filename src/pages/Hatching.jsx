    ...
    >
      ...
      {form.chicks && (
        <div style={{ background: 'var(--green-light)', borderRadius: 10, padding: 16, textAlign: 'center' }}>
          ...
          {batch.chicksExpected > 0 && (
            <div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 4 }}>
              Expected: {(batch.chicksExpected).toLocaleString()} chicks
              {chicks >= batch.chicksExpected
                ? <span style={{ color: 'var(--green)', fontWeight: 700 }}> · ✅ Target met!</span>
                : <span style={{ color: 'var(--warning)', fontWeight: 700 }}> · {batch.chicksExpected - chicks} short of target</span>
              }
            </div>
          )}
        </div>
        ...
        <div className="grid-3">
          {hatching.map((b) => (
            <div key={b.id} className="card" style={{ borderTop: '3px solid var(--purple)' }}>
              ...
              <button className="btn btn-primary btn-full" onClick={() => setSelected(b)}>
