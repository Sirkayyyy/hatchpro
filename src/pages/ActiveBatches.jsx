...
function EditModal({ batch, onClose }) {
  const { setBatches, available, occupied } = useApp()
  const [form, setForm] = useState({ ...batch, crates: String(batch.crates), eggs: String(batch.eggs) })
  const upd = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))

  const save = () => {
    const crates = parseInt(form.crates) || 0
    const diff = crates - batch.crates
    if (batch.stage !== 'completed' && diff > available) {
      toast.error(`Not enough capacity. Only ${available} extra crate${available !== 1 ? 's' : ''} available.`)
      return
    }
    setBatches((b) => b.map((x) => x.id === batch.id ? { ...form, crates, eggs: parseInt(form.eggs) || 0 } : x))
  ...
  return (
    ...
    >
      <div className="grid-2">
        ...
        <Field label="Crates">
          <input className="input" type="number" min={1} value={form.crates} onChange={upd('crates')} />
        </Field>
        <Field label="Eggs">
          <input className="input" type="number" min={0} value={form.eggs} onChange={upd('eggs')} />
        </Field>
...
export default function ActiveBatches() {
  ...
  return (
    <div>
      ...
      <PageTitle title="Active Batches" subtitle={`${batches.length} total batch${batches.length !== 1 ? 'es' : ''} registered`} />
      ...
      {filtered.length === 0 ? (
        ...
        <div className="table-wrapper">
          <table>
            <thead>
              <tr>
                {['Batch #', 'Farm', 'Bird Type', 'Crates', 'Eggs', 'Stage', 'Date Loaded', 'Exp. Hatch', 'Actions'].map((h) => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
