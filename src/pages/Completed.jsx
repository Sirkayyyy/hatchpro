...
export default function Completed() {
  ...
  const totalExpected = total.reduce((s, b) => s + (b.chicksExpected || 0), 0)
      ...
      {total.length > 0 && (
        <div className="grid-4" style={{ marginBottom: 20 }}>
          {[
            ...
            ['Total Chicks Out', totalChicks.toLocaleString(),'🐣','var(--purple)'],
          ].map(([l, v, ic, c]) => (
      ...
      {completed.length === 0 ? (
        ...
        <div className="table-wrapper">
          <table>
            <thead>
              <tr>
                {['Batch #', 'Farm', 'Bird', 'Total Eggs', 'Fertile Eggs', 'Chicks Expected', 'Chicks Hatched', 'Hatch Rate', 'Hatch Date'].map((h) => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
