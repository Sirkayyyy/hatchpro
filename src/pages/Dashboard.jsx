  ...
  return (
    <div>
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        <StatCard label="Total Capacity"  value={capacity}  icon="🏭" color="var(--green)" sub="Maximum crates" />
        <StatCard label="Occupied"        value={occupied}  icon="📦" color="var(--amber)"  sub={`${utilPct}% utilization`} />
        <StatCard label="Available"       value={available} icon="✨" color="var(--blue)"   sub="Crates free" />
        ...
        <div className="card">
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 12, color: 'var(--text2)' }}>{occupied} of {capacity} crates occupied</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: utilPct > 80 ? 'var(--danger)' : utilPct > 60 ? 'var(--warning)' : 'var(--green)' }}>
                {utilPct}%
              </span>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {[
              ...
              <div key={l} style={{ background: 'var(--bg)', borderRadius: 8, padding: 12, textAlign: 'center' }}>
                <div style={{ fontSize: 22, fontWeight: 800, fontFamily: 'Space Grotesk', color: c }}>{v}</div>
            ...
            recent.map((b) => (
              <div key={b.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #F3F4F6' }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text2)' }}>{b.farmName} · <BirdTag type={b.birdType} /></div>
                </div>
