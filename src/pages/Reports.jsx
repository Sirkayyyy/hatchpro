...
export default function Reports() {
  ...
  // Capacity utilization over time (line) — approximate from batch load dates
  const utilData = (() => {
    const days = Array.from({ length: 14 }, (_, i) => {
      ...
      const occ = batches
        .filter((b) => b.dateLoaded && b.dateLoaded <= dateStr && (b.stage !== 'completed' || (b.hatchDate && b.hatchDate > dateStr)))
  ...
  const totalCrates    = batches.filter((b) => b.stage !== 'completed').reduce((s, b) => s + b.crates, 0)
  ...
  return (
    <div>
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        {[
          ...
          <div key={l} style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 12, padding: 18, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 26, fontWeight: 800, fontFamily: 'Space Grotesk', color: c, marginTop: 6 }}>{v}</div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={stageData} cx="50%" cy="50%" outerRadius={80} dataKey="value" labelLine={false} label={CustomLabel}>
                    {stageData.map((_, i) => (
                      <Cell key={i} fill={['#1565C0', '#F9A825', '#7B1FA2', '#2E7D32'][i % 4]} />
                    ))}
                  </Pie>
                  <Tooltip />
                </PieChart>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <SectionHeader title="Capacity Utilization (Last 14 Days)" />
      ...
      {/* Best performers + completed detail */}
      <div className="grid-2">
        {/* Best batch */}
        <div className="card">
          <SectionHeader title="Top Performers" />
          {completed.length === 0 ? (
            <EmptyState icon="🏆" title="No completed batches yet" />
          ) : (
            completed
              ...
              .map((b, i) => (
                <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid #F3F4F6' }}>
                  <div style={{
                    ...
                    background: i === 0 ? '#FFF8E1' : '#F9FAFB',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 14, fontWeight: 800, color: i === 0 ? '#F9A825' : 'var(--text3)',
                    flexShrink: 0,
                  }}>
                    {i + 1}
                  </div>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text2)' }}>{b.farmName} · {b.birdType}</div>
                  </div>
                  <div style={{
        ...
        <div className="card">
          ...
          {[
            ...
            ['Crates In Use',     totalCrates, '📦'],
            ['Total Eggs Loaded', totalEggs.toLocaleString(), '🥚'],
            ['Total Chicks Out',  totalChicks.toLocaleString(), '🐣'],
