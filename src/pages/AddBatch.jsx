...
export default function AddBatch() {
  ...
  const submit = () => {
    ...
    if (!form.batchNum || !form.farmName || !form.crates || !form.eggs || !form.dateLoaded) {
      setError('Please fill in all required fields.')
      return
    }

    const crates = parseInt(form.crates)
    if (isNaN(crates) || crates < 1) { setError('Enter a valid number of crates.'); return }

    if (occupied + crates > capacity) {
      setError(`Not enough available capacity. Only ${available} crate${available !== 1 ? 's' : ''} available.`)
      return
    }
    ...
    toast.success(`Batch ${batch.batchNum} added successfully!`)
    setSuccess(`Batch ${batch.batchNum} has been registered and is now in incubation.`)
    setForm(empty())
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          <div className="grid-2">
            ...
            <Field label="Number of Crates" required hint={`${available} crates currently available`}>
              <input className="input" type="number" min={1} max={available} placeholder="Crates" value={form.crates} onChange={upd('crates')} />
            </Field>
            <Field label="Number of Eggs" required>
              <input className="input" type="number" min={1} placeholder="Total eggs" value={form.eggs} onChange={upd('eggs')} />
            </Field>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <Field label="Remarks">
                <textarea className="input" rows={3} placeholder="Optional notes about this batch..." value={form.remarks} onChange={upd('remarks')} />
              </Field>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
            ...
            <button className="btn btn-secondary" onClick={() => { setForm(empty()); setError(''); setSuccess('') }}>Clear</button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card">
            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>Capacity Status</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 12 }}>
              {[['Total', capacity, 'var(--green)'], ['Occupied', occupied, 'var(--amber)'], ['Available', available, 'var(--blue)']].map(([l, v, c]) => (
                <div key={l} style={{ background: 'var(--bg)', borderRadius: 8, padding: 10, textAlign: 'center' }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color: c, fontFamily: 'Space Grotesk' }}>{v}</div>
